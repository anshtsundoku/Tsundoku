...
export default function Domain() {
  ...
  const [open, setOpen] = useState(null);   // post id currently open in detail view
  ...
  useEffect(() => { load(); }, [slug, tab]);

  // Subscribe to real-time updates for this domain.
  useEffect(() => {
    socket.emit('subscribe:domain', slug);
    const onNew = (post) => {
      if (tab !== 'unread') return; // new posts only appear in unread
      setPosts(prev => {
    ...
    return () => {
      socket.emit('unsubscribe:domain', slug);
  ...
  const onDismiss = async (post) => {
    // Optimistic remove — dismissed items disappear from every tab.
    setPosts(prev => prev.filter(p => p.id !== post.id));
    try { await api.dismissPost(post.id); }
    catch (e) {
      console.warn('dismiss failed, reloading', e);
      load();
    }
  };
