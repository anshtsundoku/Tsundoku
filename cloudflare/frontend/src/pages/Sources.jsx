...
export default function Sources() {
  ...
  const load = async () => {
    ...
    setDomains(ds);
    setSources(ss);
    setForm(f => ({ ...f, domain_slug: f.domain_slug || ds[0]?.slug || '' }));
  };
  ...
  return (
    <div>
      ...
      <h1 className="mt-2 text-2xl font-bold tracking-tight mb-6">Sources</h1>
              ...
              <ul className="divide-y divide-border border border-border rounded-xl overflow-hidden bg-elev">
                {d.sources.map(s => (
                  <li key={s.id} className="px-4 py-3 flex items-center gap-3">
                    <span className="text-xs font-bold text-wood uppercase tracking-wider w-20">{s.type}</span>
