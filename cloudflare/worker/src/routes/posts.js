...
export async function getPost(request, { env, params }) {
...
export async function listPosts(request, { env, url }) {
  const u = await currentUser(env, request);
...
export async function patchPost(request, { env, params }) {
  const u = await currentUser(env, request);
  ...
  args.push(params.id, u.id);
  const p = await first(env,
    `UPDATE posts SET ${sets.join(', ')} WHERE id = ? AND user_id = ? RETURNING *`,
    args
  );
  if (!p) return json({ error: 'not found' }, 404);
  return json(p);
...
export async function searchPosts(request, { env, url }) {
  const u = await currentUser(env, request);
...
export async function libraryPosts(request, { env }) {
  const u = await currentUser(env, request);
  let rows;
  try {
    rows = await all(env, `
      SELECT p.*, s.type AS source_type, s.identifier AS source_identifier,
        ...
        JOIN domains d ON d.id = p.domain_id
       WHERE p.user_id = ?
         AND (p.is_bookmarked = 1 OR p.is_weekend = 1)
         AND p.is_dismissed = 0
       ORDER BY COALESCE(p.published_at, p.ingested_at) DESC
       LIMIT 200
    `, [u.id]);
    ...
    if (/no such column.*is_weekend/i.test(e.message || '')) {
      rows = await all(env, `
        SELECT p.*, s.type AS source_type, s.identifier AS source_identifier,
          ...
          JOIN sources s ON s.id = p.source_id
          JOIN domains d ON d.id = p.domain_id
         WHERE p.user_id = ? AND p.is_bookmarked = 1 AND p.is_dismissed = 0
         ORDER BY COALESCE(p.published_at, p.ingested_at) DESC
         LIMIT 200
      `, [u.id]);
    } else throw e;
...
export async function sourceCounts(request, { env }) {
  const u = await currentUser(env, request);
  ...
  const rows = await all(env, `
    SELECT s.type AS type,
      ...
      FROM sources s
      LEFT JOIN posts p ON p.source_id = s.id AND p.user_id = ?
